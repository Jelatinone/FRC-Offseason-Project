package org.frc5411.lib.external;
...
import static org.frc5411.lib.external.EqualsUtil.*;
...
public class SwerveSetpointGenerator {
  ...
  public Setpoint generateSetpoint(
    ...
    if (limits.TranslationalVelocity() > 0.0) {
      SwerveDriveKinematics.desaturateWheelSpeeds(desiredModuleState, limits.TranslationalVelocity());
    ...
    if (all_modules_should_flip
        && !prevSetpoint.Speeds().toTwist2d().epsilonEquals(new Twist2d())
    ...
    final double max_theta_step = dt * limits.RotationalVelocity();
    for (int i = 0; i < modules.length; ++i) {
      if (!need_to_steer) {
      ...
      if (epsilonEquals(prevSetpoint.States()[i].speedMetersPerSecond, 0.0)) {
    ...
    final double max_vel_step = dt * limits.TranslationalAcceleration();
    for (int i = 0; i < modules.length; ++i) {
      if (min_s == 0.0) {
        // No need to carry on.
        break;
